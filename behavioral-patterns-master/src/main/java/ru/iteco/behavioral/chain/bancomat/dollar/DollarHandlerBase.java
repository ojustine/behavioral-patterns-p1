...
public abstract class DollarHandlerBase extends BanknoteHandler {
    ...
    private static final CurrencyType CURRENCY = CurrencyType.USD;
    ...
    public boolean validate(Banknote banknote) {
        if (banknote.getCurrency() == DollarHandlerBase.CURRENCY && banknote.getValue() == this.getValue()) {
            return true;
        }
    ...
    public boolean cash(int sum, CurrencyType currency, Map<Integer, Banknote> cashBox) {
        ...
        if (currency == DollarHandlerBase.CURRENCY) {
            int banknoteCount = sum / getValue();
            sum %= getValue();

            if (banknoteCount > 0) {
                cashBox.put(banknoteCount, new Banknote(DollarHandlerBase.CURRENCY, getValue()));
            }
            if (sum == 0) {
                return true;
            }
